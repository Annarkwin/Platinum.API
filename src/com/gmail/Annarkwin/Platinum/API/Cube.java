...
public class Cube implements ConfigurationSerializable  {
	...
	public Cube(Location m, Location n){
		this.min = new Location(m.getWorld(), Math.min(m.getX(), n.getX()), Math.min(m.getY(), n.getY()), Math.min(m.getZ(), n.getZ()));
		this.max = new Location(m.getWorld(), Math.max(m.getX(), n.getX()), Math.max(m.getY(), n.getY()), Math.max(m.getZ(), n.getZ()));
	}
		...
		if(p.getWorld() == getWorld()){
			if(p.getBlockX() >= min.getBlockX() && p.getBlockX() <= max.getBlockX()){
				if(p.getBlockY() >= min.getBlockY() && p.getBlockY() <= max.getBlockY()){
					if(p.getBlockZ() >= min.getBlockZ() && p.getBlockZ() <= max.getBlockZ()) return true;
				}
			}
		}
		return false;
