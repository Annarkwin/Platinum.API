...
public abstract class CommandHelper
{
	...
	public static void sendHelp( CommandSender sender, String[] entries, String helpname, int pagenum )
	{
		...
		sender.sendMessage(
				"§2" + helpname + " Help (page " + pagenum + " of " + CommandHelper.getPageCount(entries) + ")");
	...
	private static int getPageCount( String[] entries )
	{
		...
		return (entries.length % items_per_page == 0) ? (entries.length / items_per_page)
				: (entries.length / items_per_page + 1);
	...
	private static String[] getHelpPage( String[] entries, int pageNumber )
	{
		...
		if (entries.length != 0)
		{
			...
			if (pageNumber < getPageCount(entries))
			{
				...
				String[] page = Arrays.copyOfRange(entries, (pageNumber - 1) * items_per_page,
